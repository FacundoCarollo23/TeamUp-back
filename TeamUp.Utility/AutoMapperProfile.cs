        ...
        public AutoMapperProfile()
        {
           ...
           CreateMap<User, SesionDTO>()
            ...
            CreateMap<Event, EventUserDTO>()
               .ForMember(des =>
               des.DateTime,
               opt => opt.MapFrom(origin => origin.EventDateTime.ToString("dd/MM/yyyy H:mm"))
               );

            CreateMap<EventUserDTO, Event>()
                .ForMember(des =>
                des.EventDateTime,
                opt => opt.MapFrom(origin => DateTime.ParseExact(origin.DateTime, "dd/MM/yyyy H:mm", new CultureInfo("es-PE")))
                )
                .ForMember(destino =>
                destino.UsersEvents,
                opt => opt.Ignore()
            ...
            CreateMap<User, EventDTO>()
                .ForMember(des =>
                des.Alias,
                opt => opt.MapFrom(origin => origin.Alias));

            CreateMap<EventDTO, User>()
                .ForMember(des =>
                des.Alias,
                opt => opt.Ignore());
